<template>
  <AdminLayout active="ktv">
    ...
    <section class="filter-panel">
      ...
      <label><span>刮削状态</span><span class="select-control"><select v-model="filters.scraped" @change="search"><option value="">全部状态</option><option value="true">已刮削</option><option value="false">未刮削</option></select><ChevronDown :size="15" aria-hidden="true" /></span></label>
    ...
    <section class="table-panel">
      ...
      <div class="table-scroll"><table><thead><tr><th><input type="checkbox" :checked="allSelected" @change="toggleAll" /></th><th>歌名</th><th>歌手</th><th>类型</th><th>语种 / 标签</th><th>刮削</th><th>KTV 文件</th><th>来源</th><th>点唱</th><th class="action-cell">操作</th></tr></thead><tbody>
        <tr v-for="song in songs" :key="song.id"><td><input type="checkbox" :checked="selected.has(song.id)" @change="toggle(song.id)" /></td><td><strong>{{ song.title }}</strong></td><td>{{ song.artist }}</td><td><span class="status" :class="typeClass(song.mediaType)">{{ typeText(song.mediaType) }}</span></td><td>{{ song.language || '—' }}<small>{{ (song.tags || []).join(' / ') || '无标签' }}</small></td><td><span class="scraped-badge" :class="song.scraped?'scraped':'unscraped'">{{ song.scraped?'已刮削':'未刮削' }}</span></td><td class="path">{{ song.filePath || '—' }}</td><td>{{ sourceText(song.importSource) }}</td><td>{{ song.playCount || 0 }}</td><td class="action-cell"><div class="row-actions"><button class="link playlist-link" title="加入已有歌单" @click="openPlaylistPicker(song)"><ListPlus :size="14" />歌单</button><button class="link match-link" title="搜索、筛选并审核平台元数据" @click="goScrape([song.id],true)"><Tags :size="14" />元数据刮削</button><button class="link danger-text" title="删除歌曲" @click="deleteOne(song)"><Trash2 :size="14" />删除</button></div></td></tr>
...
const filters=reactive({keyword:'',type:'',source:'',scraped:''})
