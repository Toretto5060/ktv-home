...
async function saveAll(){ try { const updated=await api.adminPutSettings({...form}); Object.assign(form,updated); if(aiForm.enabled || aiForm.baseUrl || aiForm.bulkModel || aiForm.reasoningModel){ const config=await api.adminAiPutConfig({...aiForm,apiKey:aiForm.apiKey||null,clearApiKey:clearKey.value}); Object.assign(ai,config); } if(!aiForm.enabled){ Object.assign(aiForm,{baseUrl:ai.baseUrl||'',bulkModel:ai.bulkModel||'',reasoningModel:ai.reasoningModel||'',timeoutSeconds:ai.timeoutSeconds||60,identityThreshold:ai.identityThreshold||.97,classificationThreshold:ai.classificationThreshold||.92,jsonMode:ai.jsonMode||'AUTO',bulkConcurrency:ai.bulkConcurrency||2,reasoningConcurrency:ai.reasoningConcurrency||1}); } const music=await api.adminPutMusicSourceConfig({...musicForm});Object.assign(musicForm,{enabled:music.enabled,providers:music.providers,resultLimit:music.resultLimit,timeoutSeconds:music.timeoutSeconds,searchCacheHours:music.searchCacheHours,concurrencyLimit:music.concurrencyLimit,requestIntervalMs:music.requestIntervalMs,autoApplyThreshold:music.autoApplyThreshold});musicStatus.value=music.providerStatus||[]; aiForm.apiKey=''; clearKey.value=false; original.value=snapshot(form); aiOriginal.value=snapshot(aiForm); musicOriginal.value=snapshot(musicForm); dirty.value=false } catch(e){ await alertDialog(e.message||'保存失败') } }
